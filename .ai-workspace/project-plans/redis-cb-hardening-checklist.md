...
| 6. Verification | 🟡 | `mvn test` 548 PASS / spotless clean ✅. Phase 9.4 re-run: graceful-degradation acceptance (a) PASS (JMeter err=0%); CB-engagement (b) and recovery-convergence (d) still FAIL — root causes documented in 39-load-test-findings.md addendum; deferred as follow-ups |
...
| 2026-05-24 | Phase 9.4 `CONVERGENCE_DEADLINE_MS=2000` still tight | Med | Recovery round fires ~1s after CB-closed; listener's 2s retry can land just after | **Open — confirmed during re-run**; harness-side sleep tweak recommended as separate follow-up (out of this PR's scope) |
| 2026-05-24 | `RedisCommandTimeoutException` not in `recordExceptions` | Low | Spring Data Redis wraps it as `QueryTimeoutException` → subtype of `RedisSystemException` (already in allow-list) | **Closed via code inspection** — no config change needed |
