...
**Status:** Phases 0–9 complete (committed on `feature/redis-cache-pubsub`); Phase 9.4 is **3/4** after the 2026-07-14 follow-up — recovery-convergence and the CB exception-classification bug are fixed (see [`39-load-test-findings.md`](../../project-documentation/39-load-test-findings.md) 2026-07-14 addendum); the one remaining sub-criterion (b, CB-opens-within-30s) is a documented quick-RPS test-environment limit, not a production defect. Phases 10–11 (rollout + backward-compat cleanup) pending.
...
| 9. Load test | ✅ (1 quick-RPS caveat) | `952f0a1`, `4b8997e`, `5e047bc`, `58c91a1` | Harness + 3 production-hardening changes shipped 2026-05-24; **2026-07-14 follow-up** fixed a harness restart-policy no-op + the `QueryTimeoutException` CB-classification bug + a recovery-convergence settle race. `--quick --phase 9` results: 9.1/9.2/9.3 PASS; **9.4 now 3/4** — (a) graceful-degradation (vs genuinely-down Redis), (c), (d) recovery-convergence (40ms) PASS; (b) CB-opens-within-30s is a documented quick-RPS test-env limit (failures now register 0→20/replica; window=40 too large to flip at low RPS). Hardening tracked in [redis-cb-hardening-plan.md](redis-cb-hardening-plan.md) + [-checklist.md](redis-cb-hardening-checklist.md). |
...
- [x] Verify error rate stays at 0% — **PASS** (0 errors; 2026-07-14 re-run confirms this against a *genuinely-down* Redis, after the harness restart-policy no-op was fixed). `/execute-rule` is in-memory and untouched by Redis outage.
- [~] Verify CB engages — **quick-RPS test-env limit (not a defect)**: the two underlying bugs are fixed & proven — the `failed` CB counter now goes 0→20 per replica during the outage (was stuck at 0 due to the `QueryTimeoutException` classification gap, fixed in `58c91a1`). The CB still doesn't cross its threshold because the docker-profile window=40 can't be flipped by the ~20 failures a low-RPS quick test produces (prod high-RPS trips it in <1s). See [39-load-test-findings.md 2026-07-14 addendum](../../project-documentation/39-load-test-findings.md).
- [x] Restart Redis — `docker start drools-redis` at T+120s
- [x] Verify breaker closes; Redis hits resume — post-restart convergence **PASS**: `recovery_delta_ms=40` (≤2000) after the `PHASE9_4_RECOVERY_SETTLE_S=10` settle fix (`5e047bc`) closed the earlier race. (CB-close transition itself is vacuous because the CB never opened — see the (b) row.)
...
| 2026-05-11 | Pub/sub message loss on subscriber reconnect | Med | Logging; explicit `FixedBackOff(2s, ∞)` on `RedisMessageListenerContainer` landed in `4b8997e`; v2 Redis Streams | **Test-race Closed 2026-07-14** — the Phase 9.4 recovery-convergence sub-test now fires the recovery round after a `PHASE9_4_RECOVERY_SETTLE_S=10` settle (`5e047bc`), giving the listener's FixedBackOff retry cycle time to re-subscribe; re-run measured `recovery_delta_ms=40` (≤2000). Production behaviour still bounded to ≤2s after Redis is reachable; TTL-bounded staleness on any genuinely-missed event; documented in 35-faq.md. |
