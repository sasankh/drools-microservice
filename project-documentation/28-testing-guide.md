...
Takes ~30-60 seconds. Output ends with `BUILD SUCCESS` and a summary like `Tests run: 589, Failures: 0, Errors: 0, Skipped: 0`.
...
Counts as one entry in `mvn test` output but one `@Test` annotation in our count. (Why our count is 589 — many tests are parameterized and run multiple cases each.)
