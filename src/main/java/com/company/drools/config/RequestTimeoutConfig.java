...
public class RequestTimeoutConfig {
     ...
     */
    @Bean
    public FilterRegistrationBean<RequestTimeoutFilter> requestTimeoutFilter(TimeoutConfig timeoutConfig) {
    ...
    public static class RequestTimeoutFilter extends OncePerRequestFilter {
        ...
        protected void doFilterInternal(HttpServletRequest request, 
            ...
            // Set timeout headers
            response.setHeader("X-Request-Timeout", String.valueOf(timeoutConfig.getHttpReadTimeoutSeconds()));
            response.setHeader("X-Connection-Timeout", String.valueOf(timeoutConfig.getHttpConnectionTimeoutSeconds()));
            ...
            try {
                ...
                if (duration > TimeUnit.SECONDS.toMillis(timeoutConfig.getHttpReadTimeoutSeconds() / 2)) {
                    log.warn("Slow request detected: {} took {}ms (URI: {})", 
                               request.getMethod(), duration, request.getRequestURI());
                }
                
            } catch (Exception e) {
                long duration = System.currentTimeMillis() - startTime;
                log.error("Request failed after {}ms (URI: {}): {}", 
                            duration, request.getRequestURI(), e.getMessage());
                throw e;
            }
        }
    }
