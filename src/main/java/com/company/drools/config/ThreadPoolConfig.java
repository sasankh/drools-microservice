...
/**
 * Configuration for thread pools used in rule execution and other async operations. Provides
 * optimized thread pool settings for high-throughput rule processing.
 */
@Configuration
public class ThreadPoolConfig {
  ...
  /**
   * Thread pool executor for rule execution operations. Optimized for CPU-intensive rule processing
   * with configurable concurrency.
   */
  @Bean("ruleExecutionExecutor")
  public Executor ruleExecutionExecutor() {
    log.info(
        "Configuring rule execution thread pool: core={}, max={}, queue={}, keep-alive={}s",
        ruleExecutionCorePoolSize,
        ruleExecutionMaxPoolSize,
        ruleExecutionQueueCapacity,
        ruleExecutionKeepAliveSeconds);
  ...
  /**
   * Thread pool executor for storage operations (S3, file I/O). Optimized for I/O-bound operations
   * with separate pool to prevent blocking rule execution.
   */
  @Bean("storageExecutor")
  public Executor storageExecutor() {
    log.info(
        "Configuring storage operations thread pool: core={}, max={}, queue={}, keep-alive={}s",
        storageCorePoolSize,
        storageMaxPoolSize,
        storageQueueCapacity,
        storageKeepAliveSeconds);
