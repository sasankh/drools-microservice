...
public class CircuitBreakerConfig {
    ...
    public CircuitBreaker s3CircuitBreaker(CircuitBreakerRegistry registry) {
        io.github.resilience4j.circuitbreaker.CircuitBreakerConfig config = 
            io.github.resilience4j.circuitbreaker.CircuitBreakerConfig.custom()
                ...
                .ignoreExceptions(
                    software.amazon.awssdk.services.s3.model.NoSuchKeyException.class // Don't break on missing rules
                )
                .build();
        ...
        log.info("S3 Circuit Breaker configured: failure-rate={}%, wait-duration={}ms, window-size={}", 
    ...
    public CircuitBreaker redisCircuitBreaker(CircuitBreakerRegistry registry) {
        ...
        // Add event listeners
        circuitBreaker.getEventPublisher().onStateTransition(event -> {
            log.info("Redis Circuit breaker state transition: {} -> {}", 
                    event.getStateTransition().getFromState(), 
                    event.getStateTransition().getToState());
        });
        
        circuitBreaker.getEventPublisher().onCallNotPermitted(event -> {
            log.warn("Redis Circuit breaker call not permitted");
        });
        
        log.info("Redis Circuit Breaker configured: failure-rate={}%, wait-duration={}ms, window-size={}", 
