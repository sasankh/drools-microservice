...
public class LogSanitizer {
    ...
    private static final Set<String> SENSITIVE_KEYS = new HashSet<>(Arrays.asList(
        ...
        "credential", "ssn", "social_security_number", "credit_card", "card_number",
        "cvv", "pin", "account_number", "routing_number", "bank_account",
        "access_token", "refresh_token", "jwt", "bearer_token", "private_key",
        "client_secret", "session_id", "csrf_token", "nonce"
    ));
    ...
    public static String sanitizeMessage(String message) {
        ...
        sanitized = sanitized.replaceAll("\\b\\d{4}[\\s-]?\\d{4}[\\s-]?\\d{4}[\\s-]?\\d{4}\\b", "[CC-REDACTED]");
    ...
    public static String safeDataRepresentation(Map<String, Object> data) {
        ...
        if (sanitizedData.size() <= maxFields) {
            ...
            return sanitizedData.keySet().stream()
                    ...
                    + " (+" + (sanitizedData.size() - maxFields) + " more fields)";
        }
    }
