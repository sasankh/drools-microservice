    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if ("POST".equalsIgnoreCase(request.getMethod()) && 
            ...
            if (contentLength > validationConfig.getRequestMaxSizeBytes()) {
                log.warn("Request size {} exceeds maximum allowed size of {} bytes", 
                        contentLength, validationConfig.getRequestMaxSizeBytes());
                
                writeErrorResponse(response, "Request size exceeds maximum limit", 
                                 "Request size: " + contentLength + " bytes, Maximum allowed: " + 
                                 validationConfig.getRequestMaxSizeBytes() + " bytes");
                return;
            }
