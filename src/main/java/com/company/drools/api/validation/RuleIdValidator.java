...
public class RuleIdValidator implements ConstraintValidator<ValidRuleId, String> {
  ...
  public boolean isValid(String ruleId, ConstraintValidatorContext context) {
    ...
    if (trimmedRuleId.length() > validationConfig.getRuleIdMaxLength()) {
      addViolation(
          context,
          "Rule ID too long (max " + validationConfig.getRuleIdMaxLength() + " characters)");
      return false;
    }
    ...
    if (!RULE_ID_PATTERN.matcher(trimmedRuleId).matches()) {
      addViolation(
          context,
          "Rule ID contains invalid characters (only alphanumeric, dots, dashes, underscores allowed)");
      return false;
    }

    // Prevent path traversal attempts
    if (trimmedRuleId.contains("..")
        || trimmedRuleId.startsWith("/")
        || trimmedRuleId.contains("\\")) {
